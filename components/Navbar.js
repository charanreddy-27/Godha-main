  ...
  return (
    <nav className={`sticky top-0 z-50 transition-all duration-500 ${
      scrolled 
        ...
        : 'bg-gradient-to-b from-ivory-50 to-white/95'
    }`}>
      {/* Top Bar */}
      <div className="hidden md:block bg-gradient-to-r from-royal-700 via-royal-600 to-royal-700 text-white py-2.5 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 flex justify-between items-center text-xs tracking-wide relative z-10">
          ...
          <div className="flex items-center gap-6">
            <span className="hover:text-gold-300 transition-colors cursor-pointer">📞 +91 98765 43210</span>
            <span className="hover:text-gold-300 transition-colors cursor-pointer">✉️ care@godhacollections.com</span>
          </div>
        </div>
      </div>
        ...
        <div className="flex justify-between items-center h-20 md:h-24">
          ...
          <Link href="/" className="flex items-center group">
            <div className="relative">
              <div className="absolute -inset-2 bg-gradient-to-r from-gold-400/20 to-royal-400/20 rounded-full blur-lg opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              <img
                src="/images/godha-logo.png"
                alt="Godha Collections"
                className="h-18 md:h-24 w-auto object-contain transition-transform duration-500 group-hover:scale-105 relative z-10 drop-shadow-lg"
              />
            </div>
          </Link>
          ...
          <div className="hidden lg:flex items-center space-x-1">
            {Object.values(categories).map((category) => (
              ...
              >
                ...
                >
                  {category.name}
                  <ChevronDown className={`h-4 w-4 transition-transform duration-300 ${activeDropdown === category.slug ? 'rotate-180' : ''}`} />
                  <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-gradient-to-r from-gold-400 via-gold-500 to-gold-400 transition-all duration-300 group-hover/link:w-3/4 shadow-sm shadow-gold-300"></span>
                </Link>
                
                {/* Dropdown */}
                <div className={`absolute top-full left-0 mt-1 bg-white rounded-2xl shadow-luxury-lg py-4 min-w-[260px] border border-gold-100/50 transition-all duration-300 origin-top backdrop-blur-sm ${
                  ...
                  <div className="px-4 pb-3 mb-3 border-b border-gold-200/50">
                    <span className="text-xs font-bold text-gold-700 uppercase tracking-wider flex items-center gap-2">
                      <span className="inline-block w-1 h-1 rounded-full bg-gold-500"></span>
                      Shop {category.name}
                    </span>
                  </div>
                  {category.subcategories.map((sub, idx) => (
                    ...
                    >
                      <span className="absolute left-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-gold-400 opacity-0 group-hover/item:opacity-100 transition-opacity"></span>
                      {sub.name}
                    </Link>
                  ))}
                </div>
        ...
        <div className={`lg:hidden overflow-hidden transition-all duration-500 ease-out ${
          ...
          <div className="py-6 border-t border-ivory-200 bg-white">
            {Object.values(categories).map((category, categoryIdx) => (
              <div key={category.slug} className="py-3 animate-fade-in" style={{ animationDelay: `${categoryIdx * 100}ms` }}>
                <Link
                  href={`/${category.slug}`}
            ...
            <div className="mt-6 pt-6 border-t border-ivory-200">
              <p className="text-xs text-royal-400 uppercase tracking-wider mb-3">Contact Us</p>
              <p className="text-sm text-royal-600 mb-2">📞 +91 98765 43210</p>
              <p className="text-sm text-royal-600">✉️ care@godhacollections.com</p>
            </div>
          </div>
        </div>
      </div>
